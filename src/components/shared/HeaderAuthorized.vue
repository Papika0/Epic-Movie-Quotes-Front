<template>
    <div class="absolute w-full h-4/5 bg-neutral-900 z-50 flex flex-col gap-6" v-if="showSearch">
        ...
        <p class="text-gray-300 text-opacity-60 ml-20">Enter <span class="text-white">@</span> to search movies</p>
        <p class="text-gray-300 text-opacity-60 ml-20">Enter <span class="text-white">#</span> to search quotes</p>
    </div>
    <nav class="bg-header h-86">
        <header class="flex lg:flex-row flex-row justify-between lg:mx-16 mx-9 lg:pt-6 pt-8">
            ...
            <p class="text-header my-auto lg:block hidden" @click="router.push({ name: 'home' })">MOVIE QUOTES</p>
            <div class="flex lg:flex-row flex-row-reverse gap-4 my-auto">
                <NotificationsDropDown />
                <IconSearch class="my-auto lg:hidden w-6 h-6" v-if="isNewsFeedRoute" @click="showSearch = true" />
                <LanguageDropDown />
