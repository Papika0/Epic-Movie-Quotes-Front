<template>
  <div class="flex flex-col gap-2">
    <Field :name="name" :id="name" :rules="rules" v-model="file" v-slot="{ meta }">
      ...
      <div
        class="bg-neutral-900 rounded border border-gray-500"
      ...
      >
        <img
          :src="imageURL"
          class="lg:w-433 lg:h-147px w-1/2 h-110px object-cover"
          v-if="showPhoto"
        />
        <div
          class="ml-4 justify-start items-center gap-2 lg:gap-4"
          :class="{
            'flex flex-col': showPhoto,
            'ml-4 lg:justify-start flex flex-row justify-between mr-4 lg:mr-0': !showPhoto
          }"
        >
          <p
            v-if="showPhoto"
            class="text-orange-200 font-bold text-xs lg:text-base uppercase leading-normal"
          >
