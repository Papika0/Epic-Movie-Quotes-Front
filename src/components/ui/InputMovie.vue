<template>
  <div class="flex flex-col gap-2 w-full">
    <Field :name="name" :id="name" :rules="rules" v-slot="{ field, meta }" :value="oldValue">
      <div
        class="h-12 px-4 py-2 bg-neutral-900 rounded border border-gray-500 items-center flex flex-row gap-2 relative"
        :class="
      ...
      >
        ...
        <IconInputInvalid
          :class="[name === 'year' ? 'lg:mr-4' : 'lg:mr-16', 'absolute right-0 hidden lg:block']"
        ...
        <IconInputValid
          ...
          v-if="meta.valid && meta.touched"
        />
        <label :for="name" class="text-gray-500 text-xl font-normal leading-loose" v-if="label">{{
          lang
        }}</label>
      </div>
      <IconInputValid
        class="absolute right-0 mr-2 mt-4 lg:hidden"
        v-if="meta.valid && meta.touched"
      />
      <IconInputInvalid
        class="absolute right-0 mr-2 mt-4 lg:hidden"
